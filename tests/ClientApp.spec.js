...
test("@Client first item Name", async ({ browser }) => {
  //browser, page  - defined global variable
  const context = await browser.newContext(); //we can add cookies/plugins
  const page = await context.newPage();

  page.on("request", (route) => console.log(route.url()));
  page.on("response", (route) => console.log(route.status()));
  // page.route("**/*.{jpg,png,jpeg}", (route) => route.abort()); // aborting all image calls
  ...
  const couponTxtBox = page.locator(
    "//div[@class='field small']//input[@name='coupon']"
  );
  const couponButton = page.locator(
